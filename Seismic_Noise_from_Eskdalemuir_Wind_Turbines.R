...
plot_wind_speed_by_frequency <- function(dataset, band_width = 8) {
  ...
  for (i in 1:(length(bands) - 1)) {
    ...
    plot(NULL,
         ...
         main = paste0("EKA-adjusted PSD by Wind Speed: ", freq_low, "–", 
                       freq_high, " Hz"))
...
plot_psd_by_wind_direction <- function(dataset) {
  ...
  # Set angles (rotate so 0° is at the top).
  ...
  for (deg in seq(0, 330, by = 30)) {
    ...
    text(1.15 * cos(angle), 1.15 * sin(angle), paste0(deg, "°"), cex = 0.7)
  }
...
xgb_data <- standard_welch %>%
  mutate(
    chunk_id = interaction(Date, drop = TRUE),
...
# Print background model diagnostics.
...
# Compute uplift in decibels (10 × log10-ratio difference).
...
# Export uplift curve with confidence ribbons.
ggplot(bootstrap_data, aes(x = Frequency)) +
  ...
  geom_line(aes(y = Median, color = "Median"), linewidth = 1.2) +
  geom_line(aes(y = xgb_uplift_dB, color = "Original"), linewidth = 0.7) +
  ...
  scale_color_manual(
    name = "Curve",
    values = c("Original" = "darkorange2", "Median" = "black")
  ) +
  labs(
...
# Plot directional uplift: model vs. empirical.
...
uplift_table <- bootstrap_data %>%
  ...
  transmute(
    ...
    Median_Uplift_dB = round(Median, 2),
...
key_freqs <- c(2, 3, 4, 5, 6, 7, 8, 10, 15, 20, 30, 40, 45)

# Get confidence interval table.
